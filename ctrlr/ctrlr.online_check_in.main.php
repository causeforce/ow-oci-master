...
function ctrlr_online_check_in_pub_main(&$env){
    ...
    $env['step']['skip']=array(4,9); // A list of step numbers for skipping, must be greater than 0
    ...
    if($env['flags']['is_frid_pop']===TRUE || $env['flags']['is_step_view_on_error']===TRUE){
        ...
        $env['API']['SURVEY']['ID']=2643;
        $env['API']['SURVEY']['QUESTION'][4]=3062; # Waiver Full Name
        $env['API']['SURVEY']['QUESTION'][5]=3063; # Is 18 years old or above
        $env['API']['SURVEY']['QUESTION'][2]=3064; # Health Insurance Policy Name
        $env['API']['SURVEY']['QUESTION'][3]=3065; # Health Insurance Policy Number
        $env['API']['SURVEY']['QUESTION'][6]=3066; # Upsell Accepted
        $env['API']['SURVEY']['QUESTION'][0]=3069; # Cancer Survivor
        $env['API']['SURVEY']['QUESTION'][1]=3070; # Vegetarian
        $env['API']['SURVEY']['QUESTION'][7]=3071; # Hidden Upsell Value
        $env['API']['SURVEY']['QUESTION'][8]=3072; # Hidden Safety Video Watched
        $env['API']['SURVEY']['UPSELL'][0]=1021; # Upsell Item: Pre-Registration for $25
        $env['API']['SURVEY']['UPSELL'][1]=1021; # Upsell Item: Pre-Registration for $125
        $env['API']['SURVEY']['UPSELL'][2]=1021; # Upsell Item: Pre-Registration for $250

        $env['API']['PARTICIPATION']['TYPE']['CREW']=1131;

        $env['data']['oci']['use_team_funds']=true;
