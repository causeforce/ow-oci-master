    ...
    if($reg instanceof SimpleXMLElement && $reg->getName()==='getRegistrationResponse'){
        ...
        if($frr instanceof SimpleXMLElement && $frr->getName()==='getFundraisingResponse'){
            ...
            if($funds_check['user_covering_oci']===FALSE && ((int)$reg->registration->teamId>=0) && @$env['data']['oci']['use_team_funds']===TRUE){
                ...
                if ($testing_mode) {
                    echo 'Team Id: ' . $reg->registration->teamId . '<br>';
                }
...
//        if($funds_check['result']['oci'] == 'deny' && $updated_individual_needed<=$funds_check['user_funds_raised']) {
//            if($funds_check['result']['check']=='team' && $updated_individual_needed==0) {
...
//        }
        ...
        $funds_check['result']['user_funds_raised']=$funds_check['user_funds_raised'];
